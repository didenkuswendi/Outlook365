...
async function outlook365(to,cc,subject,body) {
    ...
    await page.type('input[aria-label="Tambahkan subjek"]', subject); // Subjek email

    // Sisipkan body sebagai HTML
    await page.evaluate((htmlContent) => {
        const editor = document.querySelector('div[aria-label="Isi pesan, tekan Alt+F10 untuk keluar"]');
        if (editor) {
